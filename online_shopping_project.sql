...
--NO 1. How many total orders were placed in the last month?
SELECT COUNT(*) num_orders FROM orders
WHERE ordertime >= NOW() - INTERVAL '1 MONTH';

--NO 1b. How many total orders were placed in the last one week?
SELECT COUNT (*) num_orders FROM orders
WHERE ordertime >= NOW() - INTERVAL '7 DAYS';

--NO 1c. How many orders were placed in the last one week and what is the total revenue ($)?
SELECT '$' || sum(totalamount)::text as total_amount, COUNT(*) num_orders
FROM orders
WHERE ordertime >= NOW() - INTERVAL '7 DAYS';
...
--No 2. What is the total revenue generated by the top 3 customers?

SELECT SUM(total_amount) FROM (SELECT u.name, a.userid, a.province, o.paymentstate, SUM(o.totalamount) as total_amount
FROM users u
JOIN address a ON a.userid = u.userid
JOIN deliver_to d ON a.addrid = d.addrid
JOIN orders o ON d.ordernumber = o.ordernumber
GROUP BY u.name, a.userid, a.province, o.paymentstate 
ORDER BY total_amount DESC LIMIT 3) 
AS amount_from_top3_customers


--No 3. What are the most popular products by category?
SELECT category, product_name, COUNT(product_name) as product_count
FROM orders
...
--No 4. How many orders were cancelled in the last year?
SELECT COUNT(*) FROM orders
WHERE order_status = 'cancelled' AND order_date >= DATEADD(year, -1, GETDATE());

--No 5. What is the average order value for each month in the last year?
SELECT DATE_TRUNC('month', order_date) as month, AVG(total_price) as avg_order_value
FROM orders
WHERE order_date >= DATEADD(year, -1, GETDATE())
GROUP BY month
ORDER BY month;

--No 6. To retrieve the information of the customer who placed the most orders:
...
--No 7. To retrieve the number of orders with a total price greater than $100:
SELECT COUNT(*) count_orders_morethan_100$
FROM orders
WHERE totalamount > 100;


--No 8. To retrieve the average order total price for each customer:
SELECT customer_id, AVG(total_price) as avg_order_total
FROM orders
GROUP BY customer_id;


--No 9. To retrieve the total number of orders for each month:
SELECT DATE_TRUNC('month', order_date) as month, COUNT(*) as order_count
FROM orders
GROUP BY month;


--No 10. To retrieve the total revenue generated from all orders
SELECT SUM(total_price) FROM orders;

--No 11. To retrieve the top 5 most frequently purchased products
...
--No 12. To retrieve the total number of orders for each month
SELECT DATE_TRUNC('month', order_date) as month, COUNT(*) as order_count
FROM orders
GROUP BY month;


--No 13. SELECT the most common province among customers:
...
--No 14. SELECT the top 10 customers by total amount spent
SELECT customers.*, SUM(orders.total_price) as total_spent
FROM customers
JOIN orders
...
--No 15. SELECT customer information and join with the address information
SELECT u.name, u.phonenumber, a.province, a.city, a.streetaddr, a.postcode
FROM users u
JOIN address a ON u.userid = a.userid;


--No 16. Retrieve the number of order per payment state
SELECT paymentstate, COUNT(*) as order_count
FROM orders
GROUP BY paymentstate
ORDER BY order_count DESC;


--No 17. Retrieve the top 10 customers who placed the most orders
...
--No 18. SELECT the number of orders placed for each product brand.
SELECT brand, COUNT(*) as num_orders
FROM product
