...
--1. How many total orders were placed in the last month?
SELECT COUNT(*) num_orders FROM orders
WHERE ordertime >= NOW() - INTERVAL '1 MONTH';

--1b. How many total orders were placed in the last one week?
SELECT COUNT (*) num_orders FROM orders
WHERE ordertime >= NOW() - INTERVAL '7 DAYS';

--1c. How many orders were placed in the last one week and what is the total revenue ($)?
SELECT '$' || sum(totalamount)::text as total_amount, COUNT(*) num_orders
FROM orders
WHERE ordertime >= NOW() - INTERVAL '7 DAYS';

--2. What is the total revenue generated by the top 3 customers?
